...
## Sahib AI Project Override

When this skill is used inside the Sahib AI project, treat it as Three.js reference material only. Sahib AI is a mobile Expo/React Native game using Three.js through `expo-gl`, not a browser-only web game.

Always follow `AGENTS.md`, `docs/PHASE_PLAN.md`, and `docs/THREEJS_MOBILE_RULES.md` first. Adapt any examples to mobile. Do not introduce browser DOM UI, WASD movement, mouse aim, pointer lock, OrbitControls, manual camera zoom/rotation, or desktop-only input patterns unless Uday explicitly approves them as debug-only tooling.

Allowed Sahib AI use: scene setup, Three.js coordinate patterns, fixed camera implementation, lighting, materials, GLB/GLTF loading, animation mixer references, object pooling, and mobile performance ideas.
